...
def extract_trade_history_to_csv(log_file_path, csv_file_path):
    ...
    try:
        ...
        if not json_buffer:
            print(
                f"Error: '{trade_history_marker}' not found or JSON block could not be identified in {log_file_path}",
                file=sys.stderr,
            )
            return
        ...
        if not (json_string.startswith("[") and json_string.endswith("]")):
            print(
                "Warning: Extracted text might not be a valid JSON array. Attempting parse anyway.",
                file=sys.stderr,
            )
            ...
            if match:
                ...
                print(
                    f"Error: Could not refine JSON block from extracted text:\n{json_string}",
                    file=sys.stderr,
                )
                return
        ...
        try:
            ...
            print(
                ...
                file=sys.stderr,
            )  # Print partial string if too long
