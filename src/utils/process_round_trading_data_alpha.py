...
def process_prices_and_trades(prices_file_path, trades_file_path, output_dir="processed_data"):
    ...
    try:
        ...
        if (
            "product" in prices_df.columns
            and "symbol" in trades_df.columns
            and "product" not in trades_df.columns
        ):
        ...
        days = (
            sorted(prices_df["day"].unique()) if "day" in prices_df.columns else [1]
        )  # Default to day 1 if not specified

        # Process each day and product
        for day in days:
            day_prices = (
                prices_df[prices_df["day"] == day] if "day" in prices_df.columns else prices_df
            )
            ...
            for product in products:
                ...
                product_trades = (
                    trades_df[trades_df["product"] == product]
                    if "product" in trades_df.columns
                    else None
                )

                if product_trades is None or (
                    len(product_trades) == 0 and "symbol" in trades_df.columns
                ):
                ...
                if product_trades is not None and len(product_trades) > 0:
                    ...
                    process_group(
                        ...
                        product,
                        merged_df,
                        f"{prices_file_path}+{trades_file_path}",
                        output_dir,
                        documents,
                    )
                else:
                    ...
                    process_group(
                        day, product, product_prices, prices_file_path, output_dir, documents
                    )
...
def process_round_csv(file_path, output_dir="processed_data"):
    ...
    try:
        ...
        if "product" in df.columns and "day" in df.columns:
            ...
            print(
                f"Warning: Unrecognized CSV format for {file_path}, processing as single document"
            )
...
def process_group(day, product, group, file_path, output_dir, documents):
    ...
    if "mid_price" in group.columns:
        ...
        content += (
            f"Price range: {group['mid_price'].min():.2f} to {group['mid_price'].max():.2f}\n"
        )
    ...
    if "+" in file_path:
        ...
        source = {"prices": prices_source, "trades": trades_source}
    else:
        # Single source file
        ...
        if "price" in df.columns:
            ...
            if "quantity" in df.columns:
                ...
                if not buy_trades.empty:
                    metrics["buy_implementation_shortfall"] = (
                        (buy_trades["price"] - buy_trades["mid_price"]) * buy_trades["quantity"]
                    ).sum() / buy_trades["quantity"].sum()

                if not sell_trades.empty:
                    metrics["sell_implementation_shortfall"] = (
                        (sell_trades["mid_price"] - sell_trades["price"])
                        * abs(sell_trades["quantity"])
                    ).sum() / abs(sell_trades["quantity"]).sum()
...
def calculate_order_book_metrics(df):
    ...
    df["book_imbalance"] = (df["total_bid_volume"] - df["total_ask_volume"]) / (
    ...
    if len(bid_price_cols) >= 2 and len(ask_price_cols) >= 2:
        ...
        df["bid_slope"] = abs(
            (df["bid_price_1"] - df["bid_price_2"]) / (df["bid_volume_1"] + df["bid_volume_2"])
        )
        df["ask_slope"] = abs(
            (df["ask_price_2"] - df["ask_price_1"]) / (df["ask_volume_1"] + df["ask_volume_2"])
        )
...
def calculate_price_impact_metrics(df):
    ...
    if "bid_price_1" in df.columns and "bid_volume_1" in df.columns and "bid_price_2" in df.columns:
        ...
        df["bid_impact"] = df.apply(
            lambda row: (
                abs(row["bid_price_1"] - row["bid_price_2"])
                if row["bid_volume_1"] < standard_volume
                else 0
            ),
            axis=1,
        )
    ...
    if "ask_price_1" in df.columns and "ask_volume_1" in df.columns and "ask_price_2" in df.columns:
        df["ask_impact"] = df.apply(
            lambda row: (
                abs(row["ask_price_2"] - row["ask_price_1"])
                if row["ask_volume_1"] < standard_volume
                else 0
            ),
            axis=1,
        )
    ...
    if "price" in df.columns and "quantity" in df.columns and len(df) > 10:
        ...
        df_sorted["signed_volume"] = df_sorted[
            "quantity"
        ]  # Could improve with trade sign detection

        # Only use rows with valid data
        valid_rows = (
            df_sorted["price_change"].notna()
            & df_sorted["signed_volume"].notna()
            & (df_sorted["signed_volume"] != 0)
        )
...
def calculate_time_based_metrics(df):
    ...
    if len(df_sorted) > 1 and metrics["time_span"] > 0:
        ...
        metrics["data_frequency"] = (
            (len(df_sorted) - 1) / metrics["time_span"] if metrics["time_span"] > 0 else 0
        )
    ...
    if "mid_price" in df_sorted.columns:
        ...
        if valid_idx.any():
            df_sorted.loc[valid_idx, "price_velocity"] = (
                df_sorted.loc[valid_idx, "price_diff"] / df_sorted.loc[valid_idx, "time_diff"]
            )
            ...
            if valid_acc_idx.any():
                df_sorted.loc[valid_acc_idx, "price_acceleration"] = (
                    df_sorted.loc[valid_acc_idx, "velocity_diff"]
                    / df_sorted.loc[valid_acc_idx, "time_diff"]
                )
                metrics["avg_price_acceleration"] = df_sorted.loc[
                    valid_acc_idx, "price_acceleration"
                ].mean()
    ...
    if "mid_price" in df_sorted.columns and len(df_sorted) >= 5:
        ...
        if len(df_sorted) >= 10:
            ...
            df_sorted["rolling_vol_10"] = (
                df_sorted["mid_price"].rolling(window=10, min_periods=5).std()
            )
            ...
            df_sorted["ma_cross"] = (
                ...
                & (df_sorted["sma_5"].shift(1) <= df_sorted["sma_10"].shift(1))
            ).astype(int) - (
                (df_sorted["sma_5"] < df_sorted["sma_10"])
                & (df_sorted["sma_5"].shift(1) >= df_sorted["sma_10"].shift(1))
            ).astype(int)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process trading data from prices, trades, and observation CSV files"
    )
    ...
    parser.add_argument(
        "--observations", type=str, help="Path to observations CSV file (for Magnificent Macarons)"
    )
    parser.add_argument("--single", type=str, help="Path to a single CSV file (legacy mode)")
    parser.add_argument(
        "--output", type=str, default="processed_data", help="Output directory for processed data"
    )
    ...
    if args.prices or args.trades or args.single or args.observations:
        ...
        while True:
            ...
            if choice == "5":
                ...
                while True:
                    file_path = input(
                        "Please enter the path to your CSV file (or 'q' to return to menu): "
                    ).strip()
                    ...
                    if not file_path.endswith(".csv"):
                        print(
                            "Warning: File does not have .csv extension. Are you sure this is a CSV file?"
                        )
                    ...
                    output_dir = input(
                        "Enter output directory path (press Enter for default 'processed_data'): "
                    ).strip()
                    if not output_dir:
                ...
                prices_file = input(
                    "Please enter the path to your PRICES CSV file (or 'q' to return to menu): "
                ).strip()
                ...
                trades_file = input(
                    "Please enter the path to your TRADES CSV file (or 'q' to return to menu): "
                ).strip()
                ...
                # Observations mode
                obs_file = input(
                    "Please enter the path to your OBSERVATIONS CSV file (or 'q' to return to menu): "
                ).strip()
                if obs_file.lower() == "q":
                    continue
