...
def summarize_trading_logs(log_file_path):
    ...
    log_blocks = re.findall(
        r'{"sandboxLog": "", "lambdaLog": "(.*?)", "timestamp": (\d+)}', log_content, re.DOTALL
    )
    ...
    summary = {
        "position_trends": {
            product: [history[0], history[-1]] for product, history in position_history.items()
        },
...
if __name__ == "__main__":
    ...
    while True:
        ...
        output_file = input(
            "\nEnter the path for the summary output file (or press Enter for default 'log_summary_for_llm.md'): "
        ).strip()
        if not output_file:
