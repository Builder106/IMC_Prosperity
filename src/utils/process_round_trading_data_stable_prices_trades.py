...
def calculate_statistical_metrics(df):
    ...
    # Normality tests
    _shapiro_stat, shapiro_pval = (
        stats.shapiro(df["mid_price"]) if len(df) <= 5000 else (np.nan, np.nan)
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process trading data from prices and trades CSV files"
    )
