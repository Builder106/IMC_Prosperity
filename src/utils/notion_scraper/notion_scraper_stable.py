...
def load_code_file_mapping(mapping_file="codefile_names.md"):
    ...
    try:
        ...
        for line in lines[2:]:
            ...
            if (
                len(parts) >= 4
            ):  # With proper formatting, we should have ['', 'code_X', 'Description', '']
...
def safe_goto(
    page, url, timeout=30000, wait_until="domcontentloaded", retries=2, retry_wait_ms=1500
):
    attempt = 0
...
def process_code_content(code_content, language):
    ...
    code_content = re.sub(
        r"^(Python|JavaScript|HTML|CSS|JSON|TypeScript|Java|C\+\+|C#|Go|Rust|SQL|Bash|Shell)?\s*Copy\s*",
        "",
        code_content,
    )
    ...
    if language.lower() == "python":
        ...
        for i, line in enumerate(lines):
            ...
            if re.search(r"(\w+)\s+(\w+)", stripped_line) and ":" in stripped_line:
                ...
                if any(
                    keyword in stripped_line for keyword in indent_keywords
                ...
                is_end_statement = stripped_line.startswith("return ") or stripped_line in {
                    "return",
                    "pass",
                    "break",
                    "continue",
                ...
                next_line_not_continuation = (
                    ...
                    and not lines[i + 1]
                    .strip()
                    .startswith(("else:", "elif", "except:", "finally:"))
                )
...
def extract_content(soup, page_title):
    ...
    if title_div:
        ...
        if title_text and title_text not in seen_content:
            blocks.append({"type": "h1", "content": title_text})
    ...
    header_blocks = content_container.find_all(
        lambda tag: (
            tag.name == "h2"
            or (tag.get("class") and any("header-block" in c for c in tag.get("class")))
            or (tag.find("div", style=lambda s: s and "font-weight: 600" in s))
        )
    )
    ...
    text_blocks = content_container.find_all(
        lambda tag: (
            (tag.get("class") and any("text-block" in c for c in tag.get("class")))
            or (tag.find("div", spellcheck="true"))
        )
    )
    ...
    for block in processed_blocks:
        blocks.append({"type": block["type"], "content": block["content"]})
...
def extract_text_content_from_container(container):
    ...
    for heading in headings:
        ...
        if heading_text:
            ...
            blocks.append({"type": heading_type, "content": heading_text})
    ...
    for div in potential_paragraphs:
        ...
        if class_attr and any(
            c in " ".join(class_attr) for c in ["container", "wrapper", "layout"]
        ):
            continue
        ...
        if len(text) < 50 and (
            (
                div.get("style")
                and ("font-weight:" in div.get("style") or "font-size:" in div.get("style"))
            )
    ...
    for link in links:
        ...
        if href and not href.startswith("#"):  # Skip internal page links
            ...
            if text and len(text) > 1:
                blocks.append({"type": "link", "content": text, "href": href})
...
def extract_list_items(list_element, list_style):
    ...
    for item in list_items_elements:
        ...
        items.append({"content": text, "level": level})
...
def scrape_notion_wiki():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            notion_links = page.query_selector_all(
                ".notion-link-token a, .notion-selectable a, .notion-page-block a"
            )
        ...
        for link in internal_links:
            ...
            index.append({"title": title, "path": f"{category}/{file_name}", "source_url": link})
