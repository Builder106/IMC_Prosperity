    ...
    try:
        ...
        if "product" in df.columns and "day" in df.columns:
            ...
            for key, group in df.groupby(["day", "product"]):
                ...
                process_market_data_group(
                    day, product, group, file_path, output_dir, documents, file_type, round_info
                )
...
def process_trade_data_group(
    day, symbol, group, file_path, output_dir, documents, file_type, round_info
):
    ...
    if bid_cols and ask_cols and "bid_price_1" in df.columns and "ask_price_1" in df.columns:
        try:
            metrics["volume_weighted_price"] = (
                df["bid_volume_1"] * df["bid_price_1"] + df["ask_volume_1"] * df["ask_price_1"]
...
def calculate_trade_metrics(df):
    ...
    if "timestamp" in df.columns and len(df) > 1:
        ...
        if metrics["time_span"] > 0:
            ...
            metrics["trade_frequency"] = (
                len(df) / metrics["time_span"] if metrics["time_span"] > 0 else 0
            )
    ...
    if "price" in df.columns and "timestamp" in df.columns and len(df) > 1:
        ...
        metrics["price_change_pct"] = (
            ((last_price / first_price) - 1) * 100 if first_price > 0 else 0
        )
